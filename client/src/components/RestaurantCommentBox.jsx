// client/src/components/RestaurantCommentBox.jsx
...
export default function RestaurantCommentBox({ restaurantId, focusCommentId }) {
  ...
  async function reload() {
    try {
      const res = await http.get(`/api/restaurants/${restaurantId}/comments`);
  ...
  async function submit(e) {
    ...
    try {
      setLoading(true);
      await http.post(`/api/restaurants/${restaurantId}/comments`, {
        content: text.trim(),
  ...
  async function handleDelete(id) {
    ...
    try {
      await http.delete(`/api/restaurants/${restaurantId}/comments/${id}`);
      await reload();
    } catch (e) {
  ...
  useEffect(() => {
    if (!focusCommentId || !items?.length) return;
    const id = String(focusCommentId);
    const t = setTimeout(() => {
      const el = document.getElementById(`rcmt-${id}`);
      if (el) {
  ...
  return (
    <div className="cmt-box">
      ...
      {tree.length ? (
        ...
        <div className="muted" style={{ fontSize: 13, padding: "10px 0" }}>Chưa có bình luận nào về quán này.</div>
      )}

      {me && (
        <form className="cmt-compose" onSubmit={submit} style={{ marginTop: 16 }}>
          {replyTo && (
          ...
          <div className="cmt-composeRow">
            <textarea
              ...
              placeholder="Viết bình luận về quán này…"
              style={{ minHeight: 60 }}
            />
